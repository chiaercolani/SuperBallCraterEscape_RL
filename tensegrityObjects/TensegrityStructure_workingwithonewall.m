...
%             q = linprog(f,[],[],sparse(Aeq),sparse(beq),lb,[],[],obj.quadProgOptions);
        ...
        function dynamicsUpdate(obj,tspan,y0)
            ...
            if(isempty(obj.ySim))
                ...
                lastContactWall=obj.nodePoints(:,2:3);
            else
                y = obj.ySim;
            end
            dt = obj.delT;           
            ...
            for i = 1:round(tspan/dt)                              % calculation loop
                ...
                lastContactWall(staticNotAppliedWall,:)=yy(staticNotAppliedWall,2:3);
            end
            obj.ySim =[yy;yDot];
            ...
            function nodeXYZdoubleDot = getAccel(nodeXYZ,nodeXYZdot)
                ...
                wallx=1;
                notTouchingWall = (nodeXYZ(:,1)-wallx)<0;
                ...
                normForcesWall=(wallx-nodeXYZ(:,1)).*(Kp - Kd*nodeXYZdot(:,1));
                normForcesWall(notTouchingWall)=0;
                xyDot = nodeXYZdot(:,1:2);
                xyDotWall = nodeXYZdot(:,2:3);
                ...
                %Possible static friction against the wall
                staticFWall= kFP*(lastContactWall - nodeXYZ(:,2:3)) - kFD*xyDotWall;
                staticNotAppliedWall= (sum((staticFWall).^2,2) > (muS*normForcesWall).^2)|notTouchingWall;
                staticFWall(staticNotAppliedWall,:)=0;
                ...
                xyDotMagWall = sqrt(sum((xyDotWall).^2,2));
                wWall = (1 - exp(-kk*xyDotMagWall))./xyDotMagWall;
                wWall(xyDotMagWall<1e-9) = kk;
                dynamicFmagWall =  - muD * normForcesWall .*wWall ;
                dynamicFWall = dynamicFmagWall(:,[1 1]).* xyDotWall;
                dynamicFWall(~staticNotAppliedWall,:) = 0;
                %Tangent forces for ground contact
                tangentForcesWall = staticFWall + dynamicFWall ;
                
                % Sum all the forces given by ground and wall contact on the three axis 
                groundForces = [tangentForces(:,1)+normForcesWall tangentForces(:,2)+tangentForcesWall(:,1) normForces + 0*normForces2+tangentForcesWall(:,2)];
